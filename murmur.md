...
用 OpenCat 搭配 Shortcuts 实现了一个自动给笔记打标签的功能。对于知识管理来说，记录是开始，整理是过程，检索使用是最终目的。为了便于检索，人类创造了很多元数据，例如时间戳、标签、文件夹等，层层嵌套用于归类整理。然而，填写元数据的压力被部分前置到输入端，给用户增加了许多负担，本末倒置。人类发明计算机的目的之一是解决信息检索问题。早在1947年，科学家们就已因信息过多难以检索而感到苦恼。那个时候，一个理想的信息检索机器应该可以拥有相当于一个图书馆的知识，当用户输入问题后，能够输出与书籍相关的内容，并告知每一条内容的出处。这一理想最佳的实现形式就是人工智能。人工智能是知识管理最终的解决方案。 
